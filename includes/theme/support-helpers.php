...
function get_cpt_name($post_type)
{
    $site_extras = get_field("site_extras", "option");

    if ($post_type == "post") {
        return get_field("post_label", $site_extras);
    } elseif ($post_type == "news") {
        return get_field("news_label", $site_extras);
    } elseif ($post_type == "case-study") {
       return get_field("case_study_label", $site_extras);
    } else {
        return ucfirst($post_type);
    }
}

function get_language($proper_name = false)
{
    if ($proper_name) {
        return apply_filters('wpml_translated_language_name', NULL, apply_filters('wpml_current_language', null), 'en');
    }
    return apply_filters('wpml_current_language', null);
}

function is_data_source_page($slug)
{
    if ($slug == "templates/page-acf-site-navigation.php" || $slug == "templates/page-acf-site-extras.php") {
        return true;
    }
...
function get_post_type_translation($type)
{
    $site_extras = get_field("site_extras", "option");
...
function get_blog_head_image($post_id)
{
    ...
    foreach ($blocks as $block) {
        if ($block['blockName'] === 'acf/blog-head') {
            $fields = $block['attrs']['data'] ?? [];

            $override = isset($fields['block_override_blog_feature_image']) && $fields['block_override_blog_feature_image'] === '1';
            $custom_image = $fields['block_custom_blog_feature_image'] ?? null;

            if ($override && !empty($custom_image)) {
                $image = $custom_image;
                break;
            }
        }
